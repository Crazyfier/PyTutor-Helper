...
day = {1 : "monday", 1 : "tuesday", 3 : "wednesday", 4 : "thursday", 5 : "friday", 6 : "saturday", 7 : "sunday"}
...
def input_val(input, IfWord, IfNum):
    if input.isspace() or input == "":
        ...
        return (tempvar.isalpha() == IfWord or tempvar.isnumeric() == IfNum)\
            , input
...
def update_class_schedule(day, id):
    conn = sqlite3.connect('Class_Schedule.db')
    conn.execute(f"DELETE from {option[day]} where class_no = {id}")
    conn.commit()
    conn.close()

def remove_class_schedule(day, number, target, correction):
    conn = sqlite3.connect('Class_Schedule.db')
