...
def removing_class_tab(target_day):
    ...
    def display_class_list():
        ...
        clear()
        print("{:^60}".format("[  Currently Removing Class  ]\n"))
        print("{:^60}".format(f"[{day[target_day].title()}]"))
        print("____________________________________________________________")
...
def editing_class_tab(target_day):
    ...
    while wish_to_continue == "y":
        ...
        while input_val(class_no, True, False)[0] or int(input_val(class_no, True, False)[1]) not in use_for_validation:
            if class_no == command_to_return:
                return "y"
            class_no = str(input("Invalid input, try again: ")).replace(" ","")
        class_no = int(input_val(class_no, True, False)[1])
        ...
        while (input_val(target_option, True, False)[0]) or int(input_val(target_option, True, False)[1]) not in range(1,4):
            if target_option == command_to_return:
                return "y"
            target_option = str(input("Invalid option, its up there: "))
            ...
            while overlap == True:
                for var in Input:
                    ...
                    while input_val(hour, True, False)[0] or int(input_val(hour, True, False)[1]) not in range(1, 13):
                        if hour == command_to_return:
                            return "y"
                        hour = str(input("Invalid input, try again: ")).replace(" ","")
                    hour = str(input_val(hour, True, False)[1]).zfill(2)
                    ...
                    while input_val(minute, True, False)[0] or int(input_val(minute, True, False)[1]) not in range(0, 60):
                        if minute == command_to_return:
                            return "y"
                        minute = str(input("Invalid input, try again: ")).replace(" ","")
                    minute = str(input_val(minute, True, False)[1]).zfill(2)
                    ...
                    while input_val(am_or_pm, False, True)[0] or str(input_val(am_or_pm, False, True)[1]).lower() not in ["am", "pm"]:
                        if am_or_pm == command_to_return:
                            return "y"
                        am_or_pm = str(input("Type either AM or PM (lower case is accepted): ")).replace(" ","")
                    am_or_pm = str(input_val(am_or_pm, False, True)[1]).lower()
                ...
                for row in list(conn.execute(f"SELECT class_no, start, end from {day[target_day]}_class")):
                    if time_validator(start_input, end_input, row[1], row[2]):
                        if row[0] != class_no:
                            print("")
                            print("{:^55}".format("[  Error  ]"))
                            print("Time inputed overlaps with existing classes, try again.")
                            overlap = True
